...
ex.func = function(params, callback) {
  users.login(params.emailAddress, params.password, function(response) {
    ...
    else callback({loginToken: jwt.sign({loginToken: functions.randomString(32), userId: response.id, role: response.role}, configs.key), userId: response.id, role: response.role});
  });
};
